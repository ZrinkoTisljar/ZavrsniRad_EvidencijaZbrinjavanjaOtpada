...
public class WorkOrderService {
    ...
    public WorkOrder createForUser(String userEmail, WorkOrderCreateRequest req) {
        ...
        // LOKACIJA SE VIŠE NE TRAŽI OVDJE!
        // Admin će ju dodijeliti naknadno.
        CollectionPoint cp = null;
        if (req.getCollectionPointId() != null) {
            cp = collectionPointRepo.findById(req.getCollectionPointId())
                    .orElseThrow(() -> new IllegalArgumentException("CollectionPoint not found"));
            if (!cp.isActive()) {
        ...
        wo.setPickupAddress(req.getPickupAddress());
