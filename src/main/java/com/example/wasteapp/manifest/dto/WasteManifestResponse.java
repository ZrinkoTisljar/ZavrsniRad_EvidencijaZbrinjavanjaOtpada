...
public class WasteManifestResponse {
   ...
   // private String collectionPointName;
    ...
    private String pickupAddress; // dodano: adresa
    private String userName;      // Dodano za pravo ime/tvrtku
    ...
    public WasteManifestResponse(WasteManifest wm) {
        ...
        // POPRAVLJENO: Čita se izravno adresa preuzimanja iz WorkOrdera
        this.pickupAddress = wm.getWorkOrder().getPickupAddress();

        // POPRAVLJENO: Dohvaća točno ime ili naziv tvrtke (kao kod radnih naloga)
        if (wm.getWorkOrder().getUser().getFullName() != null && !wm.getWorkOrder().getUser().getFullName().isEmpty()) {
            this.userName = wm.getWorkOrder().getUser().getFullName();
        } else {
            this.userName = wm.getWorkOrder().getUser().getCompanyName();
        }
